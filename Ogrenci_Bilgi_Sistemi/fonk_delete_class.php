...
if (isset($_POST['deleteSumbit'])) {
  if (isset($_POST['silmeOnayi']) && $_POST['silmeOnayi'] === 'Evet') {
    ...
    $sql = "DELETE FROM exams WHERE class_id = :id";
    $SORGU = $DB->prepare($sql);
    $SORGU->bindParam(':id', $id, PDO::PARAM_INT);


    $sql = "DELETE FROM classes_students WHERE class_id = :id";
    ...
    if ($SORGU->execute() && $SORGU_2->execute() && $SORGU_3->execute()) {
  ...
  <div class="row">
    ...
    border-radius: 20px;">
      ...
      <form method="POST">
        ...
        <a href="classes_info.php" class="btn btn-warning">Geri Dön</a>
      </form>
    </div>
  </div>
</div>
